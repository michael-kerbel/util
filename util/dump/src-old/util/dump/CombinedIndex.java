...
public class CombinedIndex<E> {
   ...
   public CombinedIndex( GroupIndex<E>... indexes ) {
      if ( indexes == null ) throw new IllegalArgumentException("The indexes of an CombinedIndex may not be null.");
      if ( indexes.length < 2 ) throw new IllegalArgumentException("You need at least two indexes for a CombinedIndex.");
      _indexes = indexes;
   ...
   public synchronized Iterable<E> lookup( Object... keys ) {
      if ( keys == null ) throw new IllegalArgumentException("Keys for CombinedIndex.lookup(keys) may not be null");
         ...
         if ( _searchOnHasNext ) {
            ...
            while ( !allArraysEqual || _i[0] == -1 || _indexes[0]._dump._deletedPositions.contains(_pos[0][_i[0]]) ) {
               ...
               for ( int i = 1, length = _pos.length; i < length; i++ ) {
                  ...
                  if ( _pos[i][_i[i]] > pos0 ) allArraysEqual = false;
               }
            }
            _hasNext = true;
            _searchOnHasNext = false;

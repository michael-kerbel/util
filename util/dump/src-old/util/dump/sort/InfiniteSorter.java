...
public class InfiniteSorter<E> implements Iterable<E> {
    ...
    * memory before proceeding with swaping to the specified <code>File</code> folder 
    * object or by dafault to the temporal OS directory.  
    */
   ...
   protected List<DumpInput<E>> getSegments() throws IOException {
   ...
   private void init( int maxItemsInMemory, Comparator<E> comparator, TempFileProvider tempFileProvider ) {
      ...
      if ( _maxItemsInMemory < 1 ) throw new IllegalArgumentException("maxItemsInMemory must be positive: " + maxItemsInMemory);

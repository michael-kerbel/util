...
@Ignore("abstract test base class")
public class AbstractGroupIndexTest {

   protected static final String DUMP_FILENAME = "DumpTest.dmp";
   protected static final int    DUMP_SIZE     = 100000;
   ...
   protected void testIndex( String fieldName, TestConfiguration config ) throws Exception {
      ...
      int n = 0;
      Object k = config.createKey(DUMP_SIZE / 10 + 100);
   ...
   protected void testLookup( TestConfiguration config, Field field, NonUniqueIndex index, int expectedGroupNumber ) throws IllegalAccessException,
      ...
      for ( int j = 0; j < READ_NUMBER; j++ ) {
         int id = _random.nextInt(DUMP_SIZE / 10) + 1;
         ...
         Assert.assertEquals("wrong number of elements in group " + k, expectedGroupNumber, n);
      }
   ...
   protected void testLookupAfterUpdates( TestConfiguration config, Field field, NonUniqueIndex index ) throws IllegalAccessException,
      ...
      t = System.currentTimeMillis();
      for ( int j = 0; j < READ_NUMBER; j++ ) {
         id = _random.nextInt(DUMP_SIZE / 10) + 1;
         if ( _random.nextBoolean() ) id = -id;
   ...
   public static class Bean extends Externalizer {
      ...
      public boolean equals( Object obj ) {
         ...
         if ( _groupExternalizable == null ) {
            if ( other._groupExternalizable != null ) return false;
         } else if ( !_groupExternalizable.equals(other._groupExternalizable) ) return false;
         if ( _groupInt != other._groupInt ) return false;
         if ( _groupLong != other._groupLong ) return false;
         if ( _groupLongObject == null ) {
            if ( other._groupLongObject != null ) return false;
         } else if ( !_groupLongObject.equals(other._groupLongObject) ) return false;
         if ( _groupString == null ) {
            if ( other._groupString != null ) return false;
         } else if ( !_groupString.equals(other._groupString) ) return false;
         return true;
      }
